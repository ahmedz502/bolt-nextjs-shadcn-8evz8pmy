          ...
          {features.map((feature, index) => (
            <Card key={index} className="holographic-card">
              <CardContent className="p-6">
                <div className="mb-4">{React.cloneElement(feature.icon, { className: 'h-8 w-8 text-electric-purple' })}</div>
                <h3 className="text-xl font-semibold mb-2 text-electric-purple">{feature.title}</h3>
                <p className="text-stark-white/80">{feature.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
